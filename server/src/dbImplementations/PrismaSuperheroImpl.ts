 ...
 */
export default class PrismaSuperheroImpl implements ICrudRepository<Superhero>, IGetRepository<Superhero>
{
   ...
   async getAll(offset?: number, take?:number): Promise<Superhero[]> {
       return prismaClient.superHero.findMany({skip: offset, take: take});
   }
   async delete(itemId: string): Promise<Superhero> {
       return prismaClient.superHero.delete({where:{id:itemId}})
   }
   async update(itemId: string, itemPart: Partial<Superhero>): Promise<Superhero> {
       return prismaClient.superHero.update({where:{id:itemId}, data:itemPart})
   }
}